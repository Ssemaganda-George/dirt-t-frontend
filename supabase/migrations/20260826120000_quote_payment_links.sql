...
-- gen_random_bytes() requires pgcrypto (already present on Travel Tails; IF NOT EXISTS is idempotent).
...
BEGIN
  ...
  v_guest_name := trim(p_guest_name);
  v_guest_email := lower(trim(p_guest_email));
  v_guest_phone := trim(p_guest_phone);
  IF v_guest_name = '' OR v_guest_email = '' OR v_guest_phone = '' THEN
    RETURN jsonb_build_object('success', false, 'error', 'guest_contact_required');
  END IF;

  SELECT category_id, vendor_id INTO v_cat, v_svc_vendor FROM public.services WHERE id = p_service_id;
  IF v_cat IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'service_not_found');
  END IF;
  IF v_svc_vendor IS DISTINCT FROM p_vendor_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'vendor_service_mismatch');
  END IF;
  ...
  INSERT INTO public.quotes (
    ...
    v_guest_name, v_guest_email, v_guest_phone, COALESCE(p_line_items, '[]'::jsonb),
  ...
  -- 15-arg overload: p_service_id, p_vendor_id, p_booking_date, p_guests, p_total_amount,
  -- p_tourist_id, p_service_date, p_currency, p_special_requests, p_guest_name, p_guest_email,
  -- p_guest_phone, p_pickup_location, p_dropoff_location, p_pricing_base_amount
...
REVOKE ALL ON FUNCTION public.next_quote_invoice_no() FROM PUBLIC, anon;
