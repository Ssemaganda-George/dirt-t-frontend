-- Settlement must not run for pending/unpaid bookings or incomplete MarzPay payments.
...
BEGIN
  ...
  IF p_reference IS NOT NULL THEN
    ...
    -- If a MarzPay payments row exists for this reference, it must be completed
  ...
  -- Bookings must be confirmed/completed AND payment_status = paid
...
COMMENT ON FUNCTION public.process_payment_with_commission IS
  'Settles a paid booking/order: requires booking paid+confirmed when p_booking_id set; MarzPay reference must be completed if payments row exists.';
