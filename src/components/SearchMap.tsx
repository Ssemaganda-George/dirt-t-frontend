    ...
    >
      <Popup>
        <div className="text-sm">
          ...
          {locationName && (
            <div className="text-xs text-gray-800 mb-2 font-medium">{locationName}</div>
          )}
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm max-h-60 overflow-auto">
          {suggestions.map((s, idx) => (
            <button key={idx} type="button" onClick={() => {
              ...
              <div className="text-xs text-gray-500">{s.address?.country || ''}</div>
            </button>
          ))}
        </div>
      )}
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
              />
            </div>
            <a className="text-xs text-gray-600 hover:text-gray-800 inline-block" target="_blank" rel="noreferrer" href={`https://www.openstreetmap.org/?mlat=${coords.lat}&mlon=${coords.lon}#map=16/${coords.lat}/${coords.lon}`}>View on OpenStreetMap ↗</a>
          </div>
        </div>
      )}
      ...
      {!viewOnly && coords && editableLocation && (
        <div className="flex justify-end gap-4 pt-4 border-t border-gray-200">
          <button
            onClick={clearSearch}
            className="px-6 py-3 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 hover:shadow-sm"
          >
            Clear
          </button>
          <button
