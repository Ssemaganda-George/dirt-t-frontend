                ...
                <div>
                  ...
                  <div className="border-t pt-4">
                        ...
                        return (
                          <div key={ticketType.id} className="flex justify-between items-center py-3 px-4 bg-gray-50 rounded-lg">
                            <div className="text-sm flex-1">
                              ...
                              <div className="text-xs text-gray-500">{formatCurrency(ticketType.price, order.currency)} each</div>
                            </div>
                            <div className="flex items-center gap-2">
                              {showAllTickets ? (
                                <>
                            ...
                            <div className="text-sm font-medium ml-4">{formatCurrency(ticketType.price * quantity, order.currency)}</div>
                          </div>
                        )
                      })}
                    </div>
                    ...
                    <div className="border-t pt-4 space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        <div>Subtotal</div>
                        <div className="font-medium">{formatCurrency(Number(order.total_amount || 0), order.currency)}</div>
                      </div>

                      <div className="flex justify-between text-sm">
                        <div>Service Fees</div>
                        <div className="font-medium">{formatCurrency(Math.max(1000, Math.round(Number(order.total_amount || 0) * 0.01)), order.currency)}</div>
                      </div>

                      <div className="flex justify-between text-lg font-bold mt-4 pt-3 border-t">
                        <div>Total</div>
                        <div>{formatCurrency(Number(order.total_amount || 0) + Math.max(1000, Math.round(Number(order.total_amount || 0) * 0.01)), order.currency)}</div>
                      </div>
                    </div>
                  </div>
                </div>
