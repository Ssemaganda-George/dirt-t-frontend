...
export default function OffsetDonation() {
  ...
  const [amount, setAmount] = useState<string>(String(suggestedAmount(trees)))
    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
        ...
        <div className="flex items-center gap-3 mb-3">
          <input
            ...
            placeholder={`Suggested $${suggestedAmount(trees)}`}
          />
          <button
            onClick={() => setAmount(String(suggestedAmount(trees)))}
        ...
        <p className="text-sm text-gray-600">Contributions support tree planting and local conservation projects.</p>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <Link
          ...
          to={`/conservation/checkout?kg=${encodeURIComponent(kg)}&trees=${encodeURIComponent(trees)}&suggested=${encodeURIComponent(amount)}`}
        >
          Donate
        </Link>
      </div>
