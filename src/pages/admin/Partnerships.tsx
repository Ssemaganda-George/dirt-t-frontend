...
const Partnerships: React.FC = () => {
  ...
  const handleRequestStatus = async (id: string, status: string) => {
    if (updatingRequests.has(id)) return; // Prevent multiple clicks
    ...
    try {
      ...
      // Reset filter to 'all' so user can see the status change
      setStatusFilter('all');
    } catch (error) {
      ...
      {filter === 'requests' ? (
        <div>
          ...
          <div className="mb-4 flex gap-2 flex-wrap">
            ...
            >
              Pending ({partnerRequests.filter(req => req.status === 'pending').length})
            </button>
            <button
            ...
            >
              Approved ({partnerRequests.filter(req => req.status === 'approved').length})
            </button>
            <button
            ...
            >
              Rejected ({partnerRequests.filter(req => req.status === 'rejected').length})
            </button>
          </div>
          {loading ? (
                  ...
                  {selectedRequest.status === 'pending' && (
                    <>
                      <button
                        ...
                        disabled={updatingRequests.has(selectedRequest.id)}
                      >
                      ...
                      <button
                        ...
                        disabled={updatingRequests.has(selectedRequest.id)}
                      >
                        {updatingRequests.has(selectedRequest.id) ? 'Rejecting...' : '❌ Reject Request'}
                      </button>
                    </>
                  )}
