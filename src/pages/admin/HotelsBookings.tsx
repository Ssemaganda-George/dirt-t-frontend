...
export function HotelsBookings() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string>('cat_hotels') // Pre-filtered to hotels
  ...
  useEffect(() => {
    ...
    if (!searchQuery.trim()) {
      ...
      if (selectedVendor === 'all' && selectedCategory === 'cat_hotels') {
  ...
  } else if (selectedCategory === 'cat_hotels') {
    // Filter to only show hotels bookings by default
    displayBookings = bookings.filter(booking =>
      booking.service?.service_categories?.name?.toLowerCase() === 'homes & stays' ||
      booking.service?.category_id === 'cat_hotels'
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Hotels Bookings Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage and track all hotel bookings</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <SearchBar
              placeholder="Search hotels bookings by service, customer, or booking ID..."
      ...
      <div className="bg-white rounded-xl border border-slate-200 px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md bg-blue-600 text-white">Hotels</span>
        </div>
      </div>

      <div className="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
        <div className="border-b border-slate-100 px-5 py-3">
          <h3 className="text-sm font-semibold text-slate-900">All Hotel Bookings</h3>
                ...
                {isFiltering ? (
                  ...
                  displayBookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                        {booking.service?.check_in_time
                          ? formatDateTime(booking.service?.check_in_time)
                          : 'N/A'
                        }
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-4xl shadow-xl rounded-xl bg-white">
            <div className="mt-3">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-semibold text-slate-900">Hotel Booking Details</h3>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h4 className="font-medium text-slate-900 mb-2">Hotel Service</h4>
                  <div className="space-y-2 text-sm">
                    ...
                    <p><span className="font-medium">Check-in Time:</span> {selectedBooking.service?.check_in_time ? formatDateTime(selectedBooking.service?.check_in_time) : 'N/A'}</p>
                    <p><span className="font-medium">Check-out Time:</span> {selectedBooking.service?.check_out_time ? formatDateTime(selectedBooking.service?.check_out_time) : 'N/A'}</p>
