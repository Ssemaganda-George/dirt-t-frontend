          ...
          filteredReviews.map((review) => (
            <div
              ...
              className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all hover:shadow-md ${
                review.status === 'pending' ? 'ring-2 ring-yellow-300' :
                review.status === 'rejected' ? 'opacity-60' : ''
              }`}
            >
              {/* Card Header */}
              <div className={`px-5 py-3 flex items-center justify-between ${
                review.status === 'pending' ? 'bg-yellow-50 border-b border-yellow-100' :
                review.status === 'rejected' ? 'bg-red-50 border-b border-red-100' :
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${
                    ...
                    review.status === 'rejected' ? 'bg-red-500' : 'bg-green-500'
                  }`}>
                    {(review.visitor_name || 'A')[0].toUpperCase()}
                  </div>
                  <div>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center gap-2 bg-amber-50 px-3 py-2 rounded-lg">
                    ...
                    <span className="text-lg font-bold text-amber-700">{review.rating}.0</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t">
                  {review.status === 'pending' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={actionLoading === review.id}
                        className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors font-medium"
                      >
                  ...
                  {review.status === 'approved' && (
                    <button
                      onClick={() => openRejectModal(review)}
                      className="flex items-center gap-2 px-4 py-2 text-red-600 text-sm rounded-lg hover:bg-red-50 border border-red-200 transition-colors font-medium"
                    >
                  ...
                  {review.status === 'rejected' && (
                    <button
                      onClick={() => handleApprove(review.id)}
                      className="flex items-center gap-2 px-4 py-2 text-green-600 text-sm rounded-lg hover:bg-green-50 border border-green-200 transition-colors font-medium"
                    >
                      <CheckCircle className="w-4 h-4" />
                      Restore Review
                    </button>
