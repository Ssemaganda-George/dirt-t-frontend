  ...
  if (!vendor) {
    return (
      <div className="p-8">
        ...
        <p className="mt-2 text-sm text-slate-600">The vendor could not be loaded.</p>
        <div className="mt-4">
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="text-sm text-slate-500">Applied: {new Date(vendor.created_at).toLocaleString()}</div>
        </div>
        <div className="space-x-3">
          <button onClick={() => navigate('/admin/vendors')} className="text-slate-600 hover:text-slate-900">
            <X className="h-5 w-5" />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="card p-4">
          <h3 className="font-medium text-slate-900">Owner Information</h3>
          <div className="mt-3 space-y-2 text-sm text-slate-700">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <section className="card p-4">
          ...
          {!bank ? (
            <p className="mt-2 text-sm text-slate-500">No bank details provided.</p>
          ) : (
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              ...
              <p className="flex items-center"><span className="font-medium">Account name:</span>&nbsp;{bank.bank_account_name}
                <button className="ml-3 text-slate-500" onClick={() => handleCopy(bank.bank_account_name)} title="Copy account name"><Copy className="h-4 w-4" /></button>
              </p>
              <p className="flex items-center"><span className="font-medium">Account number:</span>&nbsp;{bank.bank_account_number}
                <button className="ml-3 text-slate-500" onClick={() => handleCopy(bank.bank_account_number)} title="Copy account number"><Copy className="h-4 w-4" /></button>
              </p>
        ...
        <section className="card p-4">
          <h3 className="font-medium text-slate-900">Mobile Money Accounts</h3>
          {mobiles.length === 0 ? (
            <p className="mt-2 text-sm text-slate-500">No mobile money accounts provided.</p>
          ) : (
            <div className="mt-2 space-y-3 text-sm text-slate-700">
              {mobiles.map((m: any, idx: number) => (
                <div key={idx} className="space-y-1">
                  ...
                  {m.name && (
                    <div className="text-sm text-slate-700">Account name: <span className="font-medium">{m.name}</span>
                      <button className="ml-3 text-slate-500" onClick={() => handleCopy(m.name)} title="Copy account name"><Copy className="h-4 w-4" /></button>
                    </div>
                  )}
                  <div className="flex items-center text-xs text-slate-600"><span className="font-mono">{(m.country_code || '') + ' ' + (m.phone || '')}</span>
                    <button className="ml-3 text-slate-500" onClick={() => handleCopy((m.country_code || '') + (m.phone || ''))}><Copy className="h-4 w-4" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4">
            <h4 className="font-medium text-slate-900">Preferred payout</h4>
            <div className="mt-2 text-sm text-slate-700">{(vendor as any).preferred_payout || 'Not set'}</div>
          </div>
        </section>
      </div>
      ...
      <div className="mt-6 flex items-center space-x-3">
        ...
        <button onClick={() => navigate('/admin/vendors')} className="text-slate-600">Back to list</button>
      </div>
    </div>
  )
}
