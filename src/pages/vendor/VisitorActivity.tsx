...
export default function VendorVisitorActivity() {
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-28 bg-white rounded-xl border border-slate-200 p-5">
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="text-sm font-medium text-slate-900">Activity Data Loading</p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {/* Header */}
      <div>
        <h1 className="text-3xl md:text-4xl font-semibold text-slate-900">Visitor Activity</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-blue-200 p-5 flex flex-col justify-between min-h-[110px]">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Total Visitors</p>
          <div>
            <p className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalVisitors}</p>
            <p className="mt-1 text-xs text-slate-500">{stats.uniqueVisitors} unique</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-emerald-200 p-5 flex flex-col justify-between min-h-[110px]">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Bookings</p>
          <div>
            <p className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalBookings}</p>
            <p className="mt-1 text-xs text-slate-500">{stats.conversionRate.toFixed(1)}% conversion</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-violet-200 p-5 flex flex-col justify-between min-h-[110px]">
          ...
          <div>
            <p className="mt-2 text-2xl font-semibold text-slate-900">{stats.totalServices}</p>
        ...
        <div className="bg-white rounded-xl border border-amber-200 p-5 flex flex-col justify-between min-h-[110px]">
          ...
          <div>
            <p className="mt-2 text-2xl font-semibold text-slate-900">{stats.avgRating.toFixed(1)}</p>
            <p className="mt-1 text-xs text-slate-500">{stats.reviewsThisMonth} reviews this month</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-5">Performance Insights</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-slate-50 flex flex-col justify-between min-h-[100px]">
                <p className="text-xs font-medium text-slate-500">Booking Rate</p>
                <div>
          ...
          {stats.topServices.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">Your Services</h2>
                ...
                {stats.topServices.map((service) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-900 truncate">{service.serviceName}</p>
          ...
          {stats.servicesChecked && stats.servicesChecked.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">Most Viewed Services</h2>
              <div className="space-y-2">
                {stats.servicesChecked.map((service, idx) => (
                  <div key={service.id || idx} className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-900 truncate">{service.serviceName}</p>
                      <p className="text-xs text-slate-500">Service views & interactions</p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-semibold text-slate-900">{service.timesChecked || 0}</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-5">Visitor Demographics</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Age Groups */}
              <div>
                <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-3">Age Distribution</h3>
                {stats.ageGroups.length > 0 ? (
                  <div className="space-y-3">
                    {stats.ageGroups.map((age, idx) => (
                      <div key={idx}>
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-xs text-slate-600">{age.ageGroup}</p>
                          <p className="text-xs font-medium text-slate-900">{age.count}</p>
                        </div>
                  ...
                  <p className="text-xs text-slate-400 italic">Age data not available</p>
                )}
              </div>

              {/* Gender Distribution */}
              <div>
                <h3 className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-3">Gender Distribution</h3>
                <div className="space-y-3">
                  {Object.entries(stats.genderDistribution).map(([gender, count]) => {
                    ...
                    return (
                      <div key={gender}>
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-xs text-slate-600">{labels[gender]}</p>
                          <p className="text-xs font-medium text-slate-900">{count}</p>
                        </div>
                        <div className="w-full bg-slate-100 rounded-full h-1.5">
                          <div
          ...
          {stats.recentReviews.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">Recent Reviews</h2>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-medium text-slate-900">{review.visitorName}</p>
                          <span className="px-2 py-0.5 rounded-md text-xs font-medium bg-amber-50 text-amber-700">
                            {review.rating}/5
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 mt-0.5">{review.serviceName}</p>
                        <p className="text-xs text-slate-400 mt-0.5">
                          {new Date(review.date).toLocaleDateString()}
                        </p>
                      </div>
                      {review.helpful > 0 && (
                        <span className="text-xs text-slate-500">{review.helpful} helpful</span>
                      )}
                    </div>
                    {review.comment && (
                      <p className="text-sm text-slate-600 mt-2 pl-0 italic">
                        "{review.comment}"
                      </p>
                    )}
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-4">Demographics</h2>
            <div className="space-y-4">
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">Gender</p>
                <div className="flex gap-2">
                  {Object.entries(stats.genderDistribution).map(([gender, count]) => {
                    ...
                    return (
                      <div key={gender} className="flex-1 bg-slate-50 p-3 rounded-lg text-center">
                        <p className="text-sm font-semibold text-slate-900">{percentage}%</p>
                        <p className="text-xs text-slate-500">{labels[gender]}</p>
                      </div>
                    );
                  })}
                </div>
              ...
              {stats.ageGroups.length > 0 && (
                <div>
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">Top Age Groups</p>
                  <div className="space-y-2">
                    {stats.ageGroups.slice(0, 3).map((age, idx) => (
                      <div key={idx} className="flex items-center justify-between text-sm">
                        <span className="text-slate-600">{age.ageGroup}</span>
                        <span className="font-medium text-slate-900">{age.percentage}%</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {stats.topCountries.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">Visitor Countries</h2>
              <div className="space-y-3">
                {stats.topCountries.map((country, idx) => (
                  <div key={idx}>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-xs font-medium text-slate-700">{country.country}</p>
                      <p className="text-xs font-semibold text-slate-900">{country.count}</p>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-1.5">
                      <div
          ...
          {stats.visitorSessions && stats.visitorSessions.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-sm font-semibold text-slate-900 mb-4">Recent Visitors</h2>
                  <div className="space-y-2">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="space-y-2.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-slate-900">{stats.totalVisitors}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Unique Visitors</span>
                <span className="font-medium text-slate-900">{stats.uniqueVisitors}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Completed Bookings</span>
                <span className="font-medium text-slate-900">{stats.totalBookings}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Active Services</span>
                <span className="font-medium text-slate-900">{stats.totalServices}</span>
              </div>
              <div className="flex justify-between border-t border-slate-100 pt-2.5 mt-2.5">
                ...
                <span className="font-semibold text-slate-900">{stats.conversionRate.toFixed(1)}%</span>
              </div>
            </div>
          </div>
        </div>
