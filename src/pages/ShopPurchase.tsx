...
const defaultPaymentFields = (): MarzpayPaymentFieldsValue => ({
...
export default function ShopPurchase({ service }: { service: ShopService }) {
  ...
  const [paymentFields, setPaymentFields] = useState<MarzpayPaymentFieldsValue>(defaultPaymentFields)
  ...
  const {
    ...
    onCompleted: () => {
      ...
      setCompletedOrder({ ...pendingRef.current, status: 'confirmed', payment_status: 'paid' })
      setCurrentStep(4)
    },
    onFailed: () => {
        ...
        {currentStep === 3 && (
          ...
          <div className="bg-white rounded-xl border p-4 space-y-4">
            ...
            <MarzpayPaymentFields
              ...
              inputClassName="w-full px-3 py-3 border rounded-xl"
