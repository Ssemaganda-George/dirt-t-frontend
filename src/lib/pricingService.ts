...
export async function createServicePricingOverride(
  ...
  // Server-side validation: prevent creating an active override that overlaps
  // an existing active override for the same service
  try {
    const candidateFrom = override.effective_from ? new Date(override.effective_from) : new Date();
    const candidateUntil = override.effective_until ? new Date(override.effective_until) : null;

    const { data: existingOverrides, error: existingError } = await supabase
      .from('service_pricing_overrides')
      .select('id, effective_from, effective_until, override_enabled')
      .eq('service_id', override.service_id)
      .eq('override_enabled', true);

    if (existingError) {
      console.error('Error checking existing overrides before create:', existingError);
      throw new Error(`Failed to validate override overlaps: ${existingError.message}`);
    }

    if (existingOverrides && existingOverrides.length > 0 && payload.override_enabled) {
      for (const o of existingOverrides) {
        try {
          const oFrom = o.effective_from ? new Date(o.effective_from) : null;
          const oUntil = o.effective_until ? new Date(o.effective_until) : null;

          // Determine if intervals overlap. Treat null effective_until as infinity.
          const endsBefore = oUntil && candidateFrom && (oUntil < candidateFrom);
          const startsAfter = candidateUntil && oFrom && (candidateUntil < oFrom);

          if (!endsBefore && !startsAfter) {
            throw new Error('Overlapping active pricing override exists for this service. Disable or adjust existing overrides first.');
          }
        } catch (e) {
          // Re-throw overlap error
          if (e instanceof Error && e.message.includes('Overlapping active')) throw e;
    ...
    throw new Error('Failed to validate override overlaps');
  }
...
export async function updateServicePricingOverride(
  ...
  // Validate overlaps before applying updates: merge current row values with updates
  try {
    const { data: existingRow, error: existingRowError } = await supabase
      .from('service_pricing_overrides')
      .select('service_id, effective_from, effective_until, override_enabled')
      .eq('id', overrideId)
      .single();

    if (existingRowError || !existingRow) {
      console.error('Failed to load existing override for validation', existingRowError);
      throw new Error('Failed to validate override overlaps: existing override not found');
    }

    const candidateFrom = Object.prototype.hasOwnProperty.call(sanitizedUpdates, 'effective_from')
      ? new Date(sanitizedUpdates.effective_from)
      : (existingRow.effective_from ? new Date(existingRow.effective_from) : new Date());

    const candidateUntil = Object.prototype.hasOwnProperty.call(sanitizedUpdates, 'effective_until')
      ? (sanitizedUpdates.effective_until ? new Date(sanitizedUpdates.effective_until) : null)
      : (existingRow.effective_until ? new Date(existingRow.effective_until) : null);

    const candidateEnabled = Object.prototype.hasOwnProperty.call(sanitizedUpdates, 'override_enabled')
      ? Boolean(sanitizedUpdates.override_enabled)
      : Boolean(existingRow.override_enabled);

    if (candidateEnabled) {
      const { data: otherOverrides, error: otherErr } = await supabase
        .from('service_pricing_overrides')
        .select('id, effective_from, effective_until, override_enabled')
        .eq('service_id', existingRow.service_id)
        .eq('override_enabled', true)
        .neq('id', overrideId);

      if (otherErr) {
        console.error('Error fetching other overrides for validation:', otherErr);
        throw new Error('Failed to validate override overlaps');
      }
